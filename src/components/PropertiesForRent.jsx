...
export default function PropertiesForRent({ onNavigateHome, onSelectProperty }) {
  ...
  const [dealType, setDealType] = useState('Rent') // 'Rent' (For Rent) | 'Buy' (For Sale) | 'All'
  const [selectedType, setSelectedType] = useState('Any Type')
  const [selectedCity, setSelectedCity] = useState('Karachi')
  const [selectedArea, setSelectedArea] = useState('All Areas')
  const [priceRange, setPriceRange] = useState('Any Rent')
  ...
  // Filter listings based on rent controls
  const rentListings = useMemo(() => {
    return PROPERTIES_DATA.filter((p) => {
      // 1. Deal Type (Default 'Rent' for rental page)
      if (dealType !== 'All') {
      ...
      // 5. Rental Price Range
      if (priceRange !== 'Any Rent') {
        const rangeObj = RENT_PRICE_RANGES.find((r) => r.label === priceRange)
        if (rangeObj && p.priceInRent) {
          if (p.priceInRent < rangeObj.min || p.priceInRent > rangeObj.max) {
            return false
          }
        }
      }
  ...
  return (
    <div className="bg-[#f8f9fa] min-h-screen pb-16 font-sans">
      {/* 1. Top Filter Strip (ilaaqa.com layout) */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        ...
        <nav className="flex items-center gap-2 text-xs text-gray-500 font-medium mb-3">
          ...
          <span>&gt;</span>
          <span className="text-navy font-semibold">Properties for Rent</span>
        </nav>

        {/* Page Title & Subheading */}
        <div className="mb-6">
          <h1 className="font-heading font-bold text-2xl sm:text-3xl text-[#0F2A47]">
            Find Properties for Rent in {selectedArea !== 'All Areas' ? `${selectedArea}, Karachi` : 'Karachi'}
          </h1>
          <p className="text-xs sm:text-sm text-gray-500 font-medium mt-1">
            Explore verified rental apartments, portions, and shops in Garden West &amp; Garden East.
          </p>
          <p className="text-sm font-semibold text-gray-500 mt-2">
            <strong className="text-navy font-bold">{rentListings.length}</strong> Properties Found
          </p>
        </div>
        ...
        {rentListings.length > 0 ? (
              ...
              return (
                ...
                >
                  ...
                  <div className="relative md:w-80 lg:w-96 h-60 md:h-auto shrink-0 overflow-hidden bg-gray-100">
                    ...
                    {/* Dark Gradient Overlay with Title on Image */}
                    <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/85 via-black/40 to-transparent p-3.5 text-center">
                      <div className="inline-block bg-black/40 backdrop-blur-xs px-3 py-1 rounded">
                        ...
                        <p className="text-gold font-sans font-bold text-xs tracking-wider uppercase mt-0.5">
                          {p.size} {p.propertyType} For Rent
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-5 sm:p-6 flex-1 flex flex-col justify-between">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-2.5 line-clamp-1 italic">
                        {p.possessionNote || 'immediate possession, sweet water connection available'}
                      </p>
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-5 pt-3.5 border-t border-gray-100 flex-wrap gap-3">
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        />
                        <div>
                          <p className="text-xs font-bold text-[#0F2A47]">{p.agent?.name || 'Shamshad Ali'}</p>
                          <p className="text-[10px] text-gray-400 font-medium">
                            {p.agent?.refreshed || 'Refreshed 1 day ago'}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <a
                          href={`https://wa.me/923000000000?text=${encodeURIComponent(
                            `Assalam o Alaikum, I am interested in rental property [${p.id}]: ${p.title} (${p.price}) located at ${p.location}. Please share terms and viewing schedule.`
                          )}`}
                          target="_blank"
          ...
          <div className="bg-white rounded-2xl border-2 border-dashed border-gray-300 p-12 text-center max-w-lg mx-auto mt-8">
            ...
            <h3 className="font-heading font-bold text-lg text-navy">No Rental Properties Match Your Filters</h3>
            <p className="text-xs text-gray-500 mt-1">
              Try adjusting your rent range or choosing another area.
            </p>
            <button
              type="button"
              onClick={clearFilters}
            ...
            <div className="p-6">
              ...
              <div className="grid grid-cols-3 gap-3 my-5 bg-gray-50 p-4 rounded-xl text-center border border-gray-200">
                <div>
                  ...
                  <p className="font-sans font-bold text-base text-navy mt-0.5">
              ...
              <div className="mt-6 pt-4 border-t border-gray-200 grid sm:grid-cols-2 gap-3">
                ...
                <a
                  href={`https://wa.me/923000000000?text=${encodeURIComponent(
                    `Assalam o Alaikum, I am interested in rental property [${selectedProperty.id}]: ${selectedProperty.title} (${selectedProperty.price}) located at ${selectedProperty.location}. Please share terms and visit schedule.`
                  )}`}
                  target="_blank"
                  rel="noopener noreferrer"
